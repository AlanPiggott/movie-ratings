...
// Fetch new releases and trending content from TMDB and get their Google ratings - CI-compatible version
...
async function loadGenres() {
  ...
  genreCache = {
    movies: new Map(movieGenres.genres.map((g: any) => [g.id, g.name])),
    tv: new Map(tvGenres.genres.map((g: any) => [g.id, g.name]))
  }
