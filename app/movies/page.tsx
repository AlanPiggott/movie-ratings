...
export default function MoviesPage() {
  ...
  useEffect(() => {
    console.log('Movies Page: Filter params changed:', {
      genres,
      scoreMin,
      scoreMax,
      yearMin,
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      try {
        ...
        console.log('Movies Page: API Response:', {
  ...
  return (
    <div className="min-h-screen bg-[#0E0F13] pt-16">
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-4">
        ...
        {/* Header */}
        <div className="mb-6">
          <h1 className="text-3xl md:text-4xl font-bold text-white">All Movies</h1>
        </div>
          ...
          data.items.length > 0 ? (
            <>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
                {data.items.map((movie) => (
                  <MediaCard
                    ...
                    mediaType="MOVIE"
                    alsoLikedPercentage={movie.alsoLikedPercentage}
                  />
                ))}
              </div>
              ...
              <div className="mt-6 text-center text-gray-400 text-sm">
                Showing {((currentPage - 1) * pageSize) + 1}-{Math.min(currentPage * pageSize, data.total)} of {data.total} movies
              </div>
            </>
          ) : (
            <div className="flex flex-col items-center justify-center py-24 space-y-6">
              <div className="text-center space-y-3">
                <p className="text-2xl font-semibold text-white">No movies found</p>
                <p className="text-gray-400 max-w-md mx-auto">
                  No movies match your current filters. Try adjusting your filters or clearing them to see more results.
                </p>
              </div>
