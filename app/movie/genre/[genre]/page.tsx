...
export default function MovieGenrePage({ params }: PageProps) {
  ...
  useEffect(() => {
    const fetchGenreItems = async () => {
      try {
        setLoading(true)
        const response = await fetch(`/api/movie/${params.genre}`)
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-zinc-400">
            Browse all {genreName.toLowerCase()} movies in our collection
          </p>
        </div>
